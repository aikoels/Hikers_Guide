...
DELIMITER $$
USE `hikers_guide_to_the_galaxy`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_activities`(IN givenTrail_id int, IN Longitude float, IN Latitude float, IN givenText text)
BEGIN
...
DELIMITER $$
USE `hikers_guide_to_the_galaxy`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_geocache`(IN Longitude float, IN Latitude float, IN givenTrail_id int , IN username varchar(45), IN givenDesc text)
BEGIN
...
DELIMITER $$
USE `hikers_guide_to_the_galaxy`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_review`(IN givenUsername varchar(45), IN givenTrail_id int, IN givenReview TEXT, IN givenRating int)
BEGIN
